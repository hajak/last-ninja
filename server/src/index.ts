...
function handleMessage(ws: WebSocket, data: string): void {
  ...
  switch (message.type) {
    case 'create_room': {
      ...
      if (result.success && result.roomCode && result.qrCodeUrl) {
    ...
    case 'join_room': {
      const result = roomManager.joinRoom(message.roomCode, clientData.playerId, message.preferredRole);
      if (result.success && result.role) {
        ...
        if (room) {
          ...
          // Notify other player
          const otherRole: Role = result.role === 'dog' ? 'panda' : 'dog';
        ...
        send(ws, {
          type: 'room_error',
          timestamp: Date.now(),
          error: (result.errorCode as 'not_found' | 'full' | 'role_taken' | 'connection_failed') || 'connection_failed',
    ...
    case 'interact': {
      ...
      const result = room.handleInteraction(clientData.playerId, message.targetId, message.action, message.data);
      send(ws, {
...
setInterval(() => {
  for (const [roomCode, room] of roomManager.getRooms()) {
    if (room.isPlaying()) {
      ...
      // Check for respawn events and broadcast them
      const respawns = room.getPendingRespawns();
      for (const respawn of respawns) {
        const respawnMessage: EntityRespawnMessage = {
          type: 'entity_respawn',
          timestamp: Date.now(),
          entityId: respawn.entityId,
          role: respawn.role,
          position: respawn.position,
          reason: respawn.reason,
        };
        broadcastToRoom(roomCode, respawnMessage);
      }
