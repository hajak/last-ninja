...
import type { LevelData, TileData, TileType, InteractableConfig, PuzzleConfig, CameraNodeConfig } from '../types.js';
...
export function createVerticalSliceLevel(): LevelData {
  ...
  const fillRect = (x1: number, y1: number, x2: number, y2: number, type: TileType, elevation = 0, walkable?: boolean): void => {
    for (let y = y1; y <= y2; y++) {
      for (let x = x1; x <= x2; x++) {
  ...
  // Wall between start and B - FIXED: Full wall with NO opening
  // Previously had an opening at (5, 10) that allowed bypassing Puzzle A
  fillRect(1, 10, 9, 10, 'wall');
  // NO opening here - players must go through Puzzle A first
  ...
  const puzzles: PuzzleConfig[] = [
    {
      ...
      name: 'Laser Bypass',
      description:
        'Dog scouts ahead with the camera. Panda pushes the crate onto the pressure plate to disable the laser.',
      objectives: [
        {
          id: 'obj_a_scout',
